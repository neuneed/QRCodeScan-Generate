...
extension String
{
    ...
    func generateQRCode(size:CGFloat?,color:UIColor?,bgColor:UIColor?,logo:UIImage?,radius:CGFloat,borderLineWidth:CGFloat?,borderLineColor:UIColor?,boderWidth:CGFloat?,borderColor:UIColor?) -> UIImage
    {
        ...
        let ciImageQRCode = generateCIImage(size: size, color: color, bgColor: bgColor)
    
        // 直接ciimage 转成 uiimage有问题
//        let image = UIImage(ciImage: ciImage)
        let cgImageQRCode : CGImage! = convertCIImageToCGImage(inputImage: ciImageQRCode)
        let image = UIImage.init(cgImage: cgImageQRCode)
